...
def format_date(year, month, day, hour=0, minute=0):
    """날짜 포맷: YYYYMMDDHHMM"""
    return f"{year:04d}{month:02d}{day:02d}{hour:02d}{minute:02d}"


def get_date_ranges(year, month):
    """월을 일주일 단위로 나누기"""
    last_day = get_last_day(year, month)
    start_dt = datetime(year, month, 1, 0, 0)
    end_dt = datetime(year, month, last_day, 23, 59)

    ranges = []
    cur = start_dt

    while cur <= end_dt:
        chunk_end = min(cur + timedelta(days=RANGE_LIMIT_DAYS - 1), end_dt)

        start_date = format_date(cur.year, cur.month, cur.day, cur.hour, cur.minute)
        end_date = format_date(chunk_end.year, chunk_end.month, chunk_end.day,
                               chunk_end.hour, chunk_end.minute)
...
def get_folder_path(year, month):
    """월별 폴더 경로"""
    folder_name = f"scsbid_public_procurement_{year:04d}{month:02d}"
...
def get_prefix(year, month):
    """파일 프리픽스"""
    last_day = get_last_day(year, month)
    start = format_date(year, month, 1, 0, 0)
    end = format_date(year, month, last_day, 23, 59)
    return f"scsbid_{start}_{end}"
...
def load_status():
    """상태 파일 읽기"""
    status_file = os.path.join(BASE_DIR, "status_scsbid.json")
...
def save_status(year, month, range_idx):
    """상태 파일 저장 (현재 구간 인덱스 포함)"""
    status_file = os.path.join(BASE_DIR, "status_scsbid.json")
    status = {
        "start_year": year,
        "start_month": month,
        "current_year": year,
        "current_month": month,
        "current_range_idx": range_idx,  # 현재 구간 저장
...
def save_checkpoint(ckpt_path, range_idx, current_page, rows, requests, part_num, buffer_count, total_count):
    """체크포인트 저장"""
    data = {
        'range_idx': range_idx,
...
def write_summary(summary_path, merged_count, total_count, elapsed_sec, error_reason,
    ...
    if is_success and merged_count == total_count:
        status = "구간 완료"
    elif is_success and merged_count > 0:
        status = "구간 부분 완료"
    else:
        status = "구간 수집 중단"
...
상태: {status}
시간: {elapsed_str}
요청: {request_count:,}건 / {DAILY_LIMIT:,}건
...
def write_execution_log(log_path, year, month, range_idx, status, elapsed_sec, merged_count, total_count, reason):
    """실행 이력 로그 추가"""
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    text = f"""
[{timestamp}] 실행: {year:04d}-{month:02d} (구간 {range_idx + 1})
[{timestamp}] 상태: {status}
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == '--init':
        init_str = sys.argv[2]
        if len(init_str) != 6:
            print("사용: python bigdata_api_program_scsbd_info_sequential_dk.py --init 202510")
            sys.exit(1)
        year, month = int(init_str[:4]), int(init_str[4:6])
        save_status(year, month, 0)  # 구간 0부터 시작
        log_msg(f"초기화: {year}년 {month}월 (구간 0부터 시작)")
        sys.exit(0)
    else:
        status = load_status()
        if status is None:
            print("먼저 초기화하세요:")
            print("python bigdata_api_program_scsbd_info_sequential_dk.py --init 202510")
            sys.exit(1)
        year = status['current_year']
        month = status['current_month']
        range_idx = status.get('current_range_idx', 0)
        log_msg(f"재개: {year}년 {month}월 (구간 {range_idx + 1})")

    # 2. 경로 설정
    paths = get_file_paths(year, month)
    prefix = get_prefix(year, month)

    log_msg(f"기간: {year}년 {month}월")

    # 3. 일주일 단위로 기간 분할
    date_ranges = get_date_ranges(year, month)
    log_msg(f"전체 구간: {len(date_ranges)}개 (7일 단위)")

    # 4. 현재 구간 확인
    if range_idx >= len(date_ranges):
        log_msg(f"모든 구간 처리 완료")
        sys.exit(0)

    current_range = date_ranges[range_idx]
    start_date = current_range['start']
    end_date = current_range['end']

    log_msg(f"【구간 {range_idx + 1}/{len(date_ranges)}】 {start_date[:8]} ~ {end_date[:8]}")

    # 5. 체크포인트 확인
    seen_this_range = set()
    checkpoint = load_checkpoint(paths['checkpoint'])
    if checkpoint and checkpoint.get('range_idx') == range_idx:
        # 같은 구간 내 체크포인트가 있으면 재개
        ...
        log_msg(f"구간 {range_idx + 1} 데이터: {total_count:,}건")
        request_count = 1
    ...
    try:
        while total_rows < target_rows and request_count < DAILY_LIMIT:
            ...
            if not items:
                log_msg(f"구간 {range_idx + 1}의 모든 데이터 수집 완료")
                break

            # 데이터 저장
            req_count, saved_count = save_buffer(items, paths['buffer'], total_rows, target_rows, seen_this_range)
            ...
            # 체크포인트 저장 (현재 current_page가 다음 페이지)
            save_checkpoint(paths['checkpoint'], range_idx, current_page, total_rows, request_count,
    ...
    # 8. 병합
    merged_count = merge_parts(prefix, paths['folder'])
    ...
    write_summary(paths['summary'], merged_count, total_count, elapsed, error_reason or "정상",
                  is_success, request_count)
    write_execution_log(paths['log'], year, month, range_idx, status_msg, elapsed, merged_count,
                        total_count, error_reason or "정상")

    log_msg(f"구간 {range_idx + 1}: {status_msg} - {int(elapsed)}초")

    # 11. 다음 구간 준비
    if is_success and merged_count == total_count:
        # 구간 완료 → 다음 구간으로 이동
        next_range_idx = range_idx + 1
        save_status(year, month, next_range_idx)

        if next_range_idx < len(date_ranges):
            log_msg(f"다음: python .\bigdata_api_program_scsbd_info_sequential_dk.py (구간 {next_range_idx + 1})")
        else:
            log_msg(f"모든 구간 처리 완료")
        ...
        # 에러 또는 부분 완료 → 같은 구간 유지
        save_status(year, month, range_idx)
        log_msg(f"재실행: python .\bigdata_api_program_scsbd_info_sequential_dk.py (구간 {range_idx + 1} 계속)")
