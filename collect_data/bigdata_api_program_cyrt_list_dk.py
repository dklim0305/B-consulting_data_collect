...
# 설정
API_URL = 'https://apis.data.go.kr/1230000/ao/PubDataOpnStdService/getDataSetOpnStdCntrctInfo'
...
def format_date(year, month, day, time_part=""):
    """날짜 포맷: YYYYMMDD (시간 제외)"""
    if time_part:
...
def get_folder_path(year, month):
    """월별 폴더 경로"""
    folder_name = f"cyrt_public_procurement_{year:04d}{month:02d}"
...
def load_status():
    """상태 파일 읽기"""
    status_file = os.path.join(BASE_DIR, "status_cyrt.json")
    ...
    with open(buffer_path, "a", encoding="utf-8") as f:
        for item in items:
            ...
            if saved_count >= remaining:
                break
            # 계약정보의 primary key 사용 (cntrctNo, cntrctOrd)
            contract_no = item.get('cntrctNo', '')
            contract_ord = item.get('cntrctOrd', '')
            key = f"{contract_no}_{contract_ord}"
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == '--init':
        init_str = sys.argv[2]
        if len(init_str) != 6:
            print("사용: python contract_api_program.py --init 202510")
            sys.exit(1)
        ...
        status = load_status()
        if status is None:
            print("첫 실행: python contract_api_program.py --init 202510")
            sys.exit(1)
    ...
    last_day = get_last_day(year, month)
    start_date = format_date(year, month, 1)
